...
//###################################################################
// LCD Initialization and definitions
//###################################################################
// Initialize the LCD
LiquidCrystal_I2C lcd(0x27, 16, 2); // Change 0x27 to your LCD's I2C address if necessary

// Define LCD Messages (see enum Messages for order, 16x2 = 16 characters on each of 2 lines)
...
void handleSmoothScrolling();
void handleScrolling();
void displayMessage(const char* message);
void updateLine(int row, const char* newText, char* previousLine);
void clearLine(int row, char* previousLine);
void chkPowerButton();
void chkModeButton();
void chkTiltSensor();
void modeTemp();
...
void handleSmoothScrolling() {
  if (!scrolling || currentScrollingMessage == nullptr) {
  ...
  if (currentMillis - previousScrollMillis >= scrollInterval) {
    ...
    // Prepare a buffer with 32 characters: 16 for Line 1 and 16 for Line 2
    char buffer[33] = {0}; // 32 chars + null terminator

    // Fill buffer with spaces initially
    memset(buffer, ' ', 32);

    // Copy the message into the buffer with wrapping
    for (int i = 0; i < messageLength; i++) {
      int bufferIndex = (i + scrollPosition) % 32;
      buffer[bufferIndex] = currentScrollingMessage[i];
    }

    // Split buffer into Line 1 and Line 2
    char displayBuffer1[17] = {0};
    char displayBuffer2[17] = {0};
    strncpy(displayBuffer1, buffer, 16);
    strncpy(displayBuffer2, buffer + 16, 16);

    // Update Line 1
    for (int i = 0; i < 16; i++) {
      char newChar = displayBuffer1[i];
      if (previousLine1[i] != newChar) {
        lcd.setCursor(i, 0);
        lcd.write(newChar);
        previousLine1[i] = newChar;
      }
    }

    // Update Line 2
    for (int i = 0; i < 16; i++) {
      char newChar = displayBuffer2[i];
      if (previousLine2[i] != newChar) {
        lcd.setCursor(i, 1);
        lcd.write(newChar);
        previousLine2[i] = newChar;
      }
    }

    // Advance the scroll position
    scrollPosition = (scrollPosition + 1) % 32;
  }
}
...
void handleScrolling() {
  ...
  if (currentMillis - previousScrollMillis >= scrollInterval) {
    ...
    // Calculate the substring to display for Line 1
    char displayBuffer1[17] = {0};
    for (int i = 0; i < 16; i++) {
      int charIndex = (scrollPosition + i) % messageLength;
      displayBuffer1[i] = currentScrollingMessage[charIndex];
    }
    displayBuffer1[16] = '\0';

    // Calculate the substring to display for Line 2
    char displayBuffer2[17] = {0};
    for (int i = 0; i < 16; i++) {
      int charIndex = (scrollPosition + i + 16) % messageLength;
      displayBuffer2[i] = currentScrollingMessage[charIndex];
    }
    displayBuffer2[16] = '\0';
...
void displayMessage(const char* message) {
  ...
  if (textLength <= 16) {
    updateLine(0, message, previousLine1);
    clearLine(1, previousLine2);
    scrolling = false;
  }
  else if (textLength <= 32) {
    updateLine(0, message, previousLine1);
    updateLine(1, message + 16, previousLine2);
    scrolling = false;
  }
  else {
    // Initiate Scrolling for Long Messages
    if (!scrolling || currentScrollingMessage != message) {
      scrolling = true;
      scrollPosition = 0;
      previousScrollMillis = millis();
      currentScrollingMessage = message;
...
void updateLine(int row, const char* newText, char* previousLine) {
  for (int i = 0; i < 16; i++) {
    char newChar = (i < strlen(newText)) ? newText[i] : ' ';
    if (previousLine[i] != newChar) {
      lcd.setCursor(i, row);
      lcd.write(newChar);
      previousLine[i] = newChar;
    }
  }
}
...
void modeTemp() {
  ...
  if (isChanged) {
    snprintf(buffer, sizeof(buffer), lcdMessage[MSG_TEMP], (int)tempC, (int)tempF);
...
void modeHumidity() {
  ...
  toggleLeds(2, LOW);
  if (isChanged) {
    snprintf(buffer, sizeof(buffer), lcdMessage[MSG_HUMIDITY], (int)humidity);
    displayMessage(buffer);
    isChanged = false;
  } 
...
void modeTimer() {
  ...
  if (isChanged) {
    snprintf(buffer, sizeof(buffer), lcdMessage[MSG_TIMER], (int)timerRemaining);
    displayMessage(buffer);
    isChanged = false;
  }
